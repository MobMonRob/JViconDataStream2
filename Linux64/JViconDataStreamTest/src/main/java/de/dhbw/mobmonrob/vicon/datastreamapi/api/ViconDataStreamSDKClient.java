...
public class ViconDataStreamSDKClient{
 ...
 public boolean isLightweightSegmentDataEnabled(){
    ...
    * In der Header Datien ist auskommentiert da in Java Operatoren nicht umdefiniert werden können
    * Deshalb ist client.SetStreamMode in client.java nicht zu finden
    * 
     * void public setStreamMode(String StramMode){
          Output_SetStreamMode result = client.SetstreamMode);
      }
    /**
     ...
     */
        ...
        Output_GetFrameRateCount  result = client.GetFrameRateCount();
         if (result.getResult().equals(Result_Enum.NotConnected)) {throw new RuntimeException("GetFrameRateCount() but client was not connected!");}
       ...
       public String getFrameRateName(long FrameRateIndex){
            ...
            //if (result.getCount()>0 && result.getCount() > FrameRateIndex){
              ...
              if (result.getResult() == Result_Enum.NotConnected) throw new RuntimeException("getFrameRateName() but client is not connected!!");
...
return result.getValue();
    ...
    public double getFrameRate(){
       ...
       // System.out.println("Frame rate is "+result.getFrameRateHz());
        if (result.getResult() == Result_Enum.NoFrame) throw new RuntimeException("getFrameRate() invoked but no frame available!");
        if (result.getResult() == Result_Enum.NotConnected) 
